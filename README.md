...
- OpenAI Responses API：uses structured output when `OPENAI_API_KEY` is configured
...
Local development defaults to SQLite. If `DATABASE_URL` is set, the backend uses Postgres. If `OPENAI_API_KEY` is set, the backend uses the OpenAI Responses API with structured output; otherwise, the deterministic agent keeps the demo stable without model cost.
...
`OPENAI_CHAT_MODEL` is a backwards-compatible alias. If it is set, the backend uses it before `OPENAI_MODEL`. `SUPABASE_SERVICE_ROLE_KEY` is backend-only. Do not expose it to the frontend.
